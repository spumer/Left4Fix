...
namespace Detours
{
	int __thiscall RecomputeVersusCompletion::OnRecompute(char arg) {
		bool team = AreTeamsFlipped(*g_pDirector);
		int i = myRecompute(this);
		
		if(i > g_totalResult) g_totalResult = i;
		
		uint32_t (*piVersusSurvivorCompletion)[4] = reinterpret_cast<uint32_t(*)[4]>((unsigned char *)(this) + 984);
