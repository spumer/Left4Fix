...
namespace Detours
{
	...
	{
		int target = gamehelpers->EntityToBCompatRef(reinterpret_cast<CBaseEntity *>(this));
		IPlayerInfo *pInfo = playerhelpers->GetGamePlayer(target)->GetPlayerInfo();
		if(pInfo) {
			r_nowAlive(pInfo->GetAbsOrigin(), g_players, sizeof(g_players)/sizeof(g_players[0]));
